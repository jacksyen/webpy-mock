    ...
    def init():
        ...
        if not cursor.fetchone():
            for user in Global.GLOBAL_ACCOUNT:
                cursor.execute('INSERT INTO %s(usercode, username, querystatus, queryresultcode, address, memo, paymentmoney, paymentstatus, paymenttype, paymentresultcode, ishangup, rechangestatus, addtime, updatetime) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)' %Global.GLOBAL_TABLE_PAYMENT_USER, (user.get('userCode'), user.get('userName'), user.get('queryStatus'), user.get('queryResultCode'), user.get('address'), user.get('memo'), user.get('paymentMoney'), user.get('paymentStatus'), user.get('paymentType'), user.get('paymentResultCode'), user.get('isHangup'), user.get('rechangeStatus'), DateUtil.getDate(format='%Y-%m-%d %H:%M:%S'), DateUtil.getDate(format='%Y-%m-%d %H:%M:%S')))
                conn.commit()
        SQLite.close(conn)
