    ...
    def init():
        ...
        if not cursor.fetchone():
            for user in Global.GLOBAL_ACCOUNT:
                ...
                if user.get('flag'):
                    flag = 1
                cursor.execute('INSERT INTO %s(usercode, username, querystatus, queryresultcode, address, memo, paymentmoney, count, price, flag, paymentstatus, paymenttype, paymentresultcode, ishangup, rechangestatus, breach, addtime, updatetime) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)' %Global.GLOBAL_TABLE_PAYMENT_USER, (user.get('userCode'), user.get('userName'), user.get('queryStatus'), user.get('queryResultCode'), user.get('address'), user.get('memo'), paymentmoney, user.get('count'), user.get('price'), flag, user.get('paymentStatus'), user.get('paymentType'), user.get('paymentResultCode'), user.get('isHangup'), user.get('rechangeStatus'), '0.00', DateUtil.getDate(format='%Y-%m-%d %H:%M:%S'), DateUtil.getDate(format='%Y-%m-%d %H:%M:%S')))
                conn.commit()
        SQLite.close(conn)
