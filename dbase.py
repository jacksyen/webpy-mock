...
class SQLite:
    ...
    def init():
        ...
        cursor.execute('''CREATE TABLE IF NOT EXISTS %s(usercode text, username text, querystatus text, queryresultcode text, address text, memo text, paymentmoney real, count real, price real, paymentstatus text, paymenttype text, paymentresultcode text, ishangup int, rechangestatus text, breach text, addtime datetime, updatetime datetime)''' %(Global.GLOBAL_TABLE_PAYMENT_USER))
        ...
        if not cursor.fetchone():
            for user in Global.GLOBAL_ACCOUNT:
                paymentmoney = 0
                if user.get('count') and user.get('price'):
                    paymentmoney = float(format((user.get('count') * user.get('price')), '.2f'))
                cursor.execute('INSERT INTO %s(usercode, username, querystatus, queryresultcode, address, memo, paymentmoney, count, price, paymentstatus, paymenttype, paymentresultcode, ishangup, rechangestatus, breach, addtime, updatetime) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)' %Global.GLOBAL_TABLE_PAYMENT_USER, (user.get('userCode'), user.get('userName'), user.get('queryStatus'), user.get('queryResultCode'), user.get('address'), user.get('memo'), paymentmoney, user.get('count'), user.get('price'), user.get('paymentStatus'), user.get('paymentType'), user.get('paymentResultCode'), user.get('isHangup'), user.get('rechangeStatus'), '0.00', DateUtil.getDate(format='%Y-%m-%d %H:%M:%S'), DateUtil.getDate(format='%Y-%m-%d %H:%M:%S')))
                conn.commit()
        SQLite.close(conn)
